...
public sealed class SmsBroadcastClient : ISmsBroadcastClient
{
        ...
        CancellationToken ct = default)
    {
        ...
        using var msg = new HttpRequestMessage(HttpMethod.Post, _options.EndpointPath) { Content = content };
        ...
        try
        {
            ...
            if (!res.IsSuccessStatusCode)
            {
                ...
                // The API often encodes logical errors in the body as "ERROR: ...".
                // Treat non-2xx as request-level errors.
    ...
    {
        if (req.To is null || req.To.Count == 0) return "At least one recipient is required.";
        ...
        if (req.From is { Length: > 11 }) return "from must be 11 chars or fewer.";
        return null;
    }
    ...
    private static bool IsErrorLine(string line, out SmsRequestError? err)
    {
        ...
        var msg = line.Substring("ERROR:".Length).Trim();
